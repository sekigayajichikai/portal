# CC-SaaS — 関ヶ谷自治会ポータル（電子回覧板・関ヶ谷レポート・週次配信・LINEリッチメニュー）

> 画面上の名前は 2026-09-24 に「関ヶ谷自治会ポータル」（管理画面は「関ヶ谷ポータル 管理」）に統一。リポジトリ名 CC-SaaS とパッケージ名 `@cc-saas/shared` は旧称のまま（変えると Vercel 連携やパスに波及するため）。
