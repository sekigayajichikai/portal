...
import { AppView } from '../../../packages/shared/types';
import { 
  LayoutDashboard, 
  Users, 
  FileText, 
    ...
    <div className="flex h-screen bg-slate-50">
      ...
      >
        <div className="flex items-center gap-2 px-2 mb-8 mt-2">
          ...
          <span className="text-xl font-bold text-slate-800 tracking-tight">Community<span className="text-primary-600">Connect</span></span>
        </div>

        <nav className="space-y-1">
          <div className="px-2 mb-2 text-xs font-bold text-slate-400 uppercase tracking-wider">管理メニュー</div>
          <NavItem view={AppView.DASHBOARD} current={currentView} icon={LayoutDashboard} label="ダッシュボード" onClick={onChangeView} />
          <NavItem view={AppView.MEMBERS} current={currentView} icon={Users} label="会員管理" onClick={onChangeView} />
          <NavItem view={AppView.CIRCULAR_BOARD} current={currentView} icon={FileText} label="デジタル回覧板" onClick={onChangeView} />
          <NavItem view={AppView.FEES} current={currentView} icon={CreditCard} label="会費管理" onClick={onChangeView} />
          <NavItem view={AppView.LIFESTYLE} current={currentView} icon={Bus} label="生活・交通情報" onClick={onChangeView} />
          
          <div className="px-2 mt-6 mb-2 text-xs font-bold text-slate-400 uppercase tracking-wider">AI・広報</div>
          <NavItem view={AppView.RADIO_STATION} current={currentView} icon={Radio} label="AI ラジオ局" onClick={onChangeView} />
          <NavItem view={AppView.PUBLIC_CONTENT} current={currentView} icon={Globe} label="公開ウェブ情報" onClick={onChangeView} />
        </nav>
      </aside>
