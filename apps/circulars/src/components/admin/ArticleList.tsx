...
import { ChevronDown, ChevronUp, Calendar, Tag, Eye, EyeOff, Pin, FileText, Edit2, GripVertical, Download, Trash2, ExternalLink, Image as ImageIcon } from 'lucide-react';
...
interface ArticleListProps {
  ...
  /** 登録済みPDF一覧（記事にPDFを添付する際の選択用） */
  availablePdfs?: { url: string; label: string }[];
}

type ViewLevel = 'summary' | 'content';
          ...
          <div className="flex-1" onClick={onToggleExpand}>
            <div className="flex items-center gap-2 mb-2 flex-wrap cursor-pointer">
              ...
              {article.tags?.map((tag, idx) => (
                <span key={idx} className="px-2 py-0.5 rounded-md text-xs font-bold bg-pink-100 text-pink-700">
                  {tag}
                </span>
              ))}

              {/* 管理用日付 */}
              {article.control_date && (
...
export const ArticleList: React.FC<ArticleListProps> = ({
  ...
  const [viewLevel, setViewLevel] = useState<ViewLevel>('content');
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1 bg-slate-100 rounded-lg p-1">
              {(['summary', 'content'] as ViewLevel[]).map((level) => (
                <button
                  key={level}
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setExpandedArticles(new Set())}
              className="px-3 py-1 rounded-lg text-sm font-medium bg-slate-100 text-slate-600 hover:bg-slate-200 transition"
            >
              全て畳む
            </button>
            <button
              onClick={() => setExpandedArticles(new Set(filteredArticles.map(a => a.id)))}
              className="px-3 py-1 rounded-lg text-sm font-medium bg-slate-100 text-slate-600 hover:bg-slate-200 transition"
            >
              全て開く
            </button>
            ...
            <div className="prose-compact max-w-none text-slate-700">
              <ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]}>
                {getArticleContent(article)}
                          ...
                          <div className="flex gap-3 flex-wrap">
                            {images.map((img: any, i: number) => (
                              <a key={i} href={img.url} target="_blank" rel="noopener noreferrer"
                                className="block rounded-lg overflow-hidden border border-slate-200 hover:shadow-md transition">
                                <img src={img.url} alt={img.label || '画像'} className="w-40 h-auto max-h-32 object-cover" />
                              </a>
                            ))}
                          </div>
                        </div>
                    ...
                    {/* PDF・その他添付ファイル */}
                    {(() => {
                      const nonImages = article.attachments?.filter((a: any) => a.type !== 'image') || [];
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-xs text-slate-500">
                                      ...
                                      {!['pdf', 'link'].includes(attachment.type) && 'ファイル'}
                                    </p>
                                  </div>
                                </div>
