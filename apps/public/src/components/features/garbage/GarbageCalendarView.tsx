...
const GarbageCalendarView: React.FC<GarbageCalendarViewProps> = ({ isSimpleMode, garbageData }) => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
      <h2 className={headingClass}>{isSimpleMode ? 'ゴミ収集カレンダー' : '🗑️ ゴミ収集スケジュール'}</h2>
      {garbageData.map((item, idx) => (
        <div key={idx} className={`flex items-center gap-4 ${isSimpleMode ? 'p-4 rounded-lg border border-slate-300' : `p-5 rounded-3xl shadow-md border-l-8 ${item.color.replace('bg-', 'border-')}`} bg-white`}>
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl ${isSimpleMode ? 'bg-slate-100' : `${item.color} shadow-inner`}`}>
            {item.icon}
          </div>
          <div className="flex-1">
            <h3 className={`font-bold ${isSimpleMode ? 'text-lg text-slate-900' : 'text-slate-800'}`}>{item.type}</h3>
            <p className="text-xs text-slate-500">{item.description}</p>
            <p className={`text-sm font-black mt-1 ${isSimpleMode ? 'text-blue-800' : 'text-slate-700'}`}>次回: {item.nextDate}</p>
          </div>
          {!isSimpleMode && (
