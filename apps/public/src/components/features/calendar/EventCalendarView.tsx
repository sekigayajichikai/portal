...
const EventCalendarView: React.FC<EventCalendarViewProps> = ({ isSimpleMode, events }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div
        className={`${isSimpleMode ? 'border border-slate-300 rounded-xl' : 'rounded-3xl shadow-md'} bg-white p-4 mb-6`}
      >
              ...
              className={`h-8 flex items-center justify-center text-sm font-bold ${
                isSimpleMode
                  ? i + 1 === 24
                    ...
                    : 'text-slate-700'
                  : i + 1 === 24
                    ? 'bg-rose-400 text-white rounded-lg'
      ...
      <div className="space-y-4">
        ...
        {events.map((event) => (
          <div
            key={event.id}
            className={`bg-white p-4 flex gap-4 ${isSimpleMode ? 'rounded-lg border border-slate-300' : 'rounded-3xl shadow-sm border border-slate-100'}`}
          >
            ...
            <div className="flex-1 flex flex-col justify-center">
              <h4
                className={`font-bold leading-tight ${isSimpleMode ? 'text-lg text-slate-900' : 'text-slate-800'}`}
              >
                {event.title}
              </h4>
              ...
              <div className="flex gap-2 mt-2">
                <button
                  className={`text-[10px] font-bold px-3 py-1 ${isSimpleMode ? 'bg-slate-200 text-slate-800 rounded' : 'bg-slate-100 rounded-full text-slate-600 hover:bg-yellow-400 hover:text-slate-900 transition-all'}`}
                >
                  参加
                </button>
                {!isSimpleMode && (
