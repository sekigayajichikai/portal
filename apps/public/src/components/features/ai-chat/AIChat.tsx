  ...
  return (
    <div className={containerClass}>
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3
            className={`font-bold ${isSimpleMode ? 'text-slate-900 text-lg' : 'text-slate-800 font-extrabold'}`}
          >
            {isSimpleMode ? 'AI相談窓口' : 'AI町内会コンシェルジュ'}
          </h3>
          <p className="text-xs text-slate-500">
            {isSimpleMode
              ? '地域のことで分からないことがあればご質問ください。'
              : 'なんでも聞いてね！'}
          </p>
        </div>
      </div>
      ...
      {loading && (
        <div className="flex justify-center py-4">
          <div
            className={`w-6 h-6 border-4 border-t-transparent rounded-full animate-spin ${isSimpleMode ? 'border-slate-300 border-t-slate-800' : 'border-emerald-200 border-t-emerald-400'}`}
          ></div>
        </div>
      )}
      ...
      {response && (
        <div
          className={`rounded-xl p-4 text-sm leading-relaxed whitespace-pre-wrap animate-in fade-in slide-in-from-bottom-2 duration-500 ${isSimpleMode ? 'bg-slate-50 text-slate-800 border border-slate-200' : 'bg-emerald-50 text-slate-700'}`}
        >
          {response}
        </div>
      )}
