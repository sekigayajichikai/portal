      ...
      {navItems.map((item) => (
        <button
          ...
          className={`flex flex-col items-center justify-center px-3 transition-all duration-300 rounded-xl
            ${isSimpleMode
              ? activeTab === item.id ? 'text-blue-700 bg-blue-50 py-1' : 'text-slate-500 hover:bg-slate-50 py-1'
              : activeTab === item.id ? 'bg-yellow-400 text-slate-900 scale-110 shadow-lg py-2' : 'text-white/60 hover:text-white py-2'
            }
          `}
        >
          <span className={isSimpleMode ? 'text-2xl mb-0.5' : 'text-2xl mb-1'}>{item.icon}</span>
          <span className={`font-bold uppercase tracking-wider ${isSimpleMode ? 'text-xs' : 'text-[10px]'}`}>{item.label}</span>
        </button>
      ))}
    </nav>
  );
