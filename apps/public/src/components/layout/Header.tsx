...
const Header: React.FC<HeaderProps> = ({ isSimpleMode, toggleSimpleMode, user, setActiveTab, forceSimpleMode = false }) => {
  return (
    <header
      className={`p-6 flex justify-between items-center sticky top-0 z-30 transition-all ${isSimpleMode ? 'bg-white shadow-sm border-b border-slate-200' : 'bg-white/50 backdrop-blur-sm'}`}
    >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {user ? (
              <>
                <span
                  className={`text-sm font-bold ${isSimpleMode ? 'text-slate-700' : 'text-orange-500'}`}
                >
                  {isSimpleMode ? user.name : 'Lv.12'}
                </span>
                <div
                  className={`w-8 h-8 rounded-full overflow-hidden border-2 ${isSimpleMode ? 'border-slate-400' : 'border-orange-400'}`}
                >
