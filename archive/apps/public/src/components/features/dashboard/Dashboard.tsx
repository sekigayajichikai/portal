...
const Dashboard: React.FC<DashboardProps> = ({
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <div className="text-center space-y-2">
          <h2
            className={`text-2xl font-black ${isSimpleMode ? 'text-slate-800' : 'text-slate-800'}`}
          >
        ...
        >
          <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current">
            <path d="M24 10.304c0-5.369-5.383-9.738-12-9.738-6.616 0-12 4.369-12 9.738 0 4.814 4.269 8.846 10.036 9.608.391.084.922.258 1.057.592.121.303.079.748.038 1.04l-.184 1.126c-.059.354-.271 1.381 1.209.754 1.48-.626 8.022-4.724 8.022-4.724C22.618 16.923 24 13.801 24 10.304z" />
          </svg>
          LINEでログイン
        </button>
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in zoom-in-95 duration-500">
      {/* User Header */}
      <div
        className={`flex items-center justify-between p-4 ${isSimpleMode ? 'bg-slate-100 border border-slate-200' : 'bg-white shadow-lg'} rounded-2xl`}
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className={`font-bold ${isSimpleMode ? 'text-slate-800' : 'text-slate-900'}`}>
                {user.name}
              </h2>
            </div>
        ...
        <button
          onClick={onLogout}
          className="text-xs font-bold text-slate-400 hover:text-slate-600 bg-slate-50 px-3 py-2 rounded-lg"
        >
          ログアウト
        </button>
      </div>
      ...
      <section>
        <h3
          className={`font-black mb-3 px-1 ${isSimpleMode ? 'text-lg text-slate-800' : 'text-xl text-slate-800'}`}
        >
        ...
        <div className="space-y-3">
          {payments.map((payment) => (
            <div
              key={payment.id}
              className={`p-5 rounded-2xl flex flex-col gap-3 ${isSimpleMode ? 'bg-white border border-slate-300' : 'bg-white shadow-md border-l-4 border-l-indigo-500'}`}
            >
      ...
      <section>
        ...
        <div className="space-y-4">
          {circulars.map((circular) => (
            <div
              key={circular.id}
              className={`p-4 rounded-2xl transition-all ${isSimpleMode ? 'bg-white border border-slate-300' : 'bg-white shadow-sm hover:shadow-md'}`}
            >
              <div className="flex justify-between items-start mb-2">
                ...
                {circular.isRead ? (
                  ...
                  <span className="text-[10px] font-bold bg-rose-100 text-rose-600 px-2 py-0.5 rounded animate-pulse">
                    未読
                  </span>
                )}
              </div>
              <h4
                className={`font-bold text-slate-800 mb-2 ${isSimpleMode ? 'text-base' : 'text-lg'}`}
              >
                {circular.title}
              </h4>
              <p
                className={`text-sm text-slate-600 leading-relaxed mb-4 ${isSimpleMode ? '' : 'line-clamp-2'}`}
              >
                {circular.content}
              </p>
