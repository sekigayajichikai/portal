 ...
 *
 * 使い方（リポジトリ直下で）:
 *   node scripts/schedule-test/run-gemini.mjs "<pdfsフォルダ名>" [prompt] [model] [出力ラベル]
...
const [folder, promptSpec = 'prod', model = 'gemini-2.5-flash', label = `${promptSpec === 'prod' ? 'prod' : promptSpec.replace(/\.md$/, '')}-${model}`] =
